...
void drawScene(void) {
	...
	for (vector<Mesh*> mesh : scene.second) {
		for (Mesh* subMesh : mesh) {
			...
			glUniform3fv(uniformLight.material_ambient, 1, value_ptr(materials[subMesh->getMaterialType()].ambient));
			glUniform3fv(uniformLight.material_diffuse, 1, value_ptr(materials[subMesh->getMaterialType()].diffuse));
			glUniform3fv(uniformLight.material_specular, 1, value_ptr(materials[subMesh->getMaterialType()].specular));
			glUniform1f(uniformLight.material_shininess, materials[subMesh->getMaterialType()].shininess);
