...
void initShapeVAO(Shape* shape) {
	...
	glGenBuffers(1, shape->getNormalsVBO());
	glBindBuffer(GL_ARRAY_BUFFER, *shape->getNormalsVBO());
	glBufferData(GL_ARRAY_BUFFER, shape->getNormals()->size() * sizeof(vec3), shape->getNormals()->data(), GL_STATIC_DRAW);
